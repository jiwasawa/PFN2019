...
for epoch in range(num_epoch):
    ...
    # names from 0-1499 for training data files
    bag = np.linspace(0,Train_size-1,Train_size,dtype=int) 
    
    loss_epoch = np.zeros(num_mini_batch) # storage for mean loss for each epoch in mini-batch
    pred_epoch = np.zeros((Train_size,2)) # storage for [y_true, y_pred] for all Training data
    ...
    for mini_batch in range(num_mini_batch):
        ...
        for k,n in enumerate(bag[rand_index]):
            ...
            y_true = np.loadtxt(TrainData_path+str(n)+'_label.txt',dtype=int)
        ...
        W = W - alpha*np.mean(W_batch,axis=2)
        A = A - alpha*np.mean(A_batch, axis=1)
        b = b - alpha*np.mean(b_batch)
...
#np.save("./data/sgd_train_loss.npy",train_loss_array)
#np.save("./data/sgd_val_loss.npy",val_loss_array)
#np.save("./data/sgd_train_acc.npy",train_acc_array)
#np.save("./data/sgd_val_acc.npy",val_acc_array)
# plot results
